...
pub struct RKStateMethod {}

impl SimulationMethod for RKStateMethod {
    ...
    fn evolve_state(
        ...
        for t_index in start_index..end_index {
            ...
            let k_1: Array1<Complex64> =
                RKStateMethod::get_lindblad_operator(circuit, qubit_array, time).dot(&qubit_state);
            let k_2: Array1<Complex64> =
                RKStateMethod::get_lindblad_operator(circuit, qubit_array, time + (dt / 2.))
                    .dot(&(&qubit_state + (&k_1 * dt)));
            let k_3: Array1<Complex64> =
                RKStateMethod::get_lindblad_operator(circuit, qubit_array, time + (dt / 2.))
                    .dot(&(&qubit_state + (&k_2 * dt)));
            let k_4: Array1<Complex64> =
                RKStateMethod::get_lindblad_operator(circuit, qubit_array, time + dt)

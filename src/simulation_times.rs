...
impl SimulationTimes {
    pub fn new(duration: f64, num_iterations: usize, num_samples: usize) -> SimulationTimes {
        ...
        for i in 0..(num_samples - 1) {
            let next_times: Vec<f64> = Array1::<f64>::linspace(sample_times[i], sample_times[i + 1] - dt, num_iterations).to_vec();
            iterations_times.push(next_times);
        }
        return SimulationTimes { iteration_times: iterations_times, sample_times: sample_times, dt: dt };
    }

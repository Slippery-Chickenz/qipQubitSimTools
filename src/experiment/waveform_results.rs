...
pub struct WaveformResults {
    /// Multi-Dimensional array to store the results of the sweep in
    results: ArrayD<f64>,
}

impl WaveformResults {
    pub fn from_json(mut results_dim: Vec<usize>, num_samples: usize) -> WaveformResults {
        if num_samples > 1 {
        ...
        return WaveformResults { results };
    }
}

impl ExperimentResult for WaveformResults {
